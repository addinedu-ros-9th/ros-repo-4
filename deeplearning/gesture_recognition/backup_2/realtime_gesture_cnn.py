...
# 기존 모델 클래스 정의 (train_shift_gcn.py와 동일)
class ShiftGCN(nn.Module):
    """Shift-GCN 모델 구현 (제스처 특성 반영)"""
...
def process_frame_for_prediction(frame_data):
    ...
    features = 99  # 랜드마크(84) + 각도(15)
    
    if frames < 60:
        # 패딩
        padding = np.zeros((60 - frames, features), dtype=np.float32)
    ...
    return torch.FloatTensor(x_with_gesture).unsqueeze(0)  # (1, 6200)

def main():
    """실시간 제스처 인식 테스트"""
    print("🚀 실시간 제스처 인식 테스트 시작")
    
    # 모델 로드
    model = ShiftGCN(num_classes=3, num_frames=60, num_joints=21)
    model.load_state_dict(torch.load('best_shift_gcn_model.pth'))
    ...
    # 예측 결과 버퍼 (부드러운 표시를 위해)
    prediction_buffer = deque(maxlen=10)
    
    print("📹 카메라 시작... (q: 종료, r: 버퍼 리셋)")
    ...
    while cap.isOpened():
            ...
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if len(frame_buffer) >= 30:  # 최소 30프레임
                    try:
                        ...
                        model_input = process_frame_for_prediction(frame_data)
                        ...
                        current_prediction = actions[predicted]
        ...
        cv2.putText(img, 'Press q to quit, r to reset', 
                   (10, 430), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 128, 128), 1)
