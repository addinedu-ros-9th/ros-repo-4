...
|--------------|-----------------------------|-------------|-------------|----------------------------|--------|----------------------------------------------------------|----------------------------------------------------------|
| central → GUI| 관리자 사용중 블락           | central     | GUI         | /alert_occupied            | POST   | {"robot_id": "3"}                                        | status code                                              |
| central → GUI| 사용 가능한 상태 알림        | central     | GUI         | /alert_idle                | POST   | {"robot_id": "3"}                                        | status code                                              |
| IF-          | 길안내 완료                  | central     | GUI         | /navigating_complete       | POST   | {"robot_id": "3"}                                        |                                                          |
| GUI → central| 30초 동안 터치 없을 경우     | GUI         | central     | /alert_timeout             | POST   | {"robot_id": "3"}                                        | status code                                              |
| IF-01        | 호출 명령 (음성)             | GUI         | central     | /call_with_voice           | POST   | {"robot_id": "3"}                                        | status code                                              |
|        | 호출 명령 (스크린)           | GUI         | central     | /call_with_screen          | POST   | {"robot_id": "3"}                                        | status code                                              |
|     | 본인확인 (주민번호)          | GUI         | central     | /auth/ssn                  | POST   | {"robot_id":"3", "ssn":"000000-0000000"}                 | {"name":"임영웅", ...} or {"error":"Reservation not found"}|
| IF-02        | 본인확인 (회원번호)          | GUI         | central     | /auth/patient_id           | POST   | {"robot_id":"3", "patient_id":"00000000"}                | {"name":"임영웅", ...}                                   |
| IF-03        | 본인확인 (RFID 카드)         | GUI         | central     | /auth/rfid                 | POST   | {"robot_id":"3", "rfid":"A1B2C3D4E5"}                    | {"name":"임영웅", ...}                                   |
| IF-04        | 본인 인증 후 길 안내         | GUI         | central     | /auth/direction            | POST   | {"patient_id":"00000000", "robot_id":3, "station_id":5}  | status_code                                              |
| IF-05        | 본인 인증 없이 길 안내       | GUI         | central     | /without_auth/direction    | POST   | {"robot_id":3, "station_id":5}                           | status_code                                              |
| IF-06        | 본인인증 후 사용완료 복귀명령| GUI         | central     | /auth/robot_return         | POST   | {"patient_id":"00000000","robot_id":"3"}                 | status_code                                              |
| IF-07        | 본인인증 없이 사용완료 복귀명령| GUI       | central     | /without_auth/robot_return | POST   | {"robot_id":"3"}                                         |                                                          |
| IF-08        | 로봇 일시정지 명령           | GUI         | central     | /pause_request             | POST   | {"robot_id":"3"}                                         |                                                          |
|              | 로봇 길안내 재개             | GUI         | central     | /restart_navigation        | POST   | {"robot_id":"3"}                                         |                                                          |
|              | pause 중 로봇 작업 종료 요청 | GUI         | central     | /stop_navigating           | POST   | {"robot_id":"3"}                                         |                                                          |
| IF-09        | 맵 로봇 위치                 | GUI         | central     | /get/robot_location        | POST   | {"robot_id":"3"}                                         | {"x":5.0, "y":-1.0, "yaw":-80} (1초에 1번 요청)          |
</details>
...
| Interface ID | Function/Description         | Sender      | Receiver    | Endpoint                   | Method | Request Data                                              | Response Data / Status Code / Description                |
|--------------|-----------------------------|-------------|-------------|----------------------------|--------|----------------------------------------------------------|----------------------------------------------------------|
| central → GUI| 환자 사용중 블락             | central     | GUI         | /alert_occupied            | POST   | {"robot_id": "3"}                                        | status code                                              |
| central → GUI| 사용 가능한 상태 알림        | central     | GUI         | /alert_idle                | POST   | {"robot_id": "3"}                                        | status code                                              |
| IF-01        | 로그인                       | GUI         | central     | /auth/login                | POST   | {"admin_id":"00000000ssn", "password":"0000"}            | status code                                              |
| IF-02        | 세부 정보                    | GUI         | central     | /auth/detail               | POST   | {"admin_id":"00000000"}                                  | {"name":"임영웅","email":"hero@mail.com","hospital_name":"서울아산병원"} |
| IF-03        | 맵 로봇 위치                 | GUI         | central     | /get/robot_location        | POST   | {"robot_id":3, "admin_id":"admin1"}                      | {"x":5.0, "y":-1.0, "yaw":-0.532151} (1초에 1번 요청)    |
| IF-04        | 로봇 상태                    | GUI         | central     | /get/robot_status          | POST   | {"robot_id":3, "admin_id":"admin1"}                      | {"status":"navigating", "orig":0, "dest":3, "battery":70, "network":4} |
| IF-05        | 이용중인 환자 정보           | GUI         | central     | /get/patient_info          | POST   | {"robot_id":3, "admin_id":"admin1"}                      | {"patient_id":"00000000", "phone":"010-1111-1111", "rfid":"33F7ADEC", "name":"김환자"} |
| IF-06        | 원격 제어 요청               | GUI         | central     | /control_by_admin          | POST   | {"robot_id":3, "admin_id":"admin1"}                      |                                                          |
| IF-07        | 원격 제어 취소               | GUI         | central     | /return_command            | POST   | {"robot_id":3, "admin_id":"admin1"}                      | status code (대기장소로 이동)                            |
|              | 수동제어 요청                | GUI         | central     | /teleop_request            | POST   | {"robot_id":3, "admin_id":"admin1"}                      | status code                                              |
|              | 수동제어 명령 완수           | GUI         | central     | /teleop_complete           | POST   | {"robot_id":3, "admin_id":"admin1"}                      | status code                                              |
| IF-08        | teleop 이동 명령             | GUI         | central     | /command/move_teleop       | POST   | {"robot_id":3, "teleop_key":1, "admin_id":"admin1"}       | status code (123=uio, 456=jkl, 789=m,.)                  |
| IF-09        | 목적지 이동 명령             | GUI         | central     | /command/move_dest         | POST   | {"robot_id":3, "dest":0, "admin_id":"admin1"}            | status code                                              |
|              | 길안내 취소                  | GUI         | central     | /cancel_navigating         | POST   | {"robot_id":3, "admin_id":"admin1"}                      | status code                                              |
| IF-10        | 로봇 로그                    | GUI         | central     | /get/log_data              | POST   | {"period":"today", "start_date":"YYYY-MM-DD", "end_date":"YYYY-MM-DD", "admin_id":"admin1"} | [ { "patient_id":"00000000", "orig":0, "dest":3, "datetime":"YYYY-MM-DD HH:MM:SS" }, ... ] |
