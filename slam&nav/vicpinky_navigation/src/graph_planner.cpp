...
public:
      ...
      std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) override
  {
      ...
      node_->declare_parameter("waypoint_radius", 0.3);
...
private:
  ...
  void checkPathObstacles()
  {
      ...
      // 현재 위치에서 lookahead 거리 내의 waypoint들 찾기
      std::vector<geometry_msgs::msg::PoseStamped> lookahead_waypoints = 
          getLookaheadWaypoints();
      
      bool obstacle_detected = false;
      
      for (const auto& waypoint : lookahead_waypoints) {
          if (isObstacleNearWaypoint(waypoint)) {
              RCLCPP_WARN(node_->get_logger(), 
                  "Obstacle detected near waypoint (%.2f, %.2f)", 
