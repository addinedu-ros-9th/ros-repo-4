...
SimpleCentralServer::SimpleCentralServer() : Node("simple_central_server")
{
    RCLCPP_INFO(this->get_logger(), "Starting Simple Central Server with Manual Navigation...");
    ...
    // 상태 업데이트 타이머만 유지 (자동 명령 타이머 제거)
    status_timer_ = this->create_wall_timer(
        1s, std::bind(&SimpleCentralServer::statusTimerCallback, this));
    
    RCLCPP_INFO(this->get_logger(), "Simple Central Server with Manual Navigation initialized");
...
void SimpleCentralServer::initializeWaypoints()
{
    ...
    waypoints_["breast cancer"] = {"Breast Cancer Center", 7.64, 1.63, 180.0, "유방암 센터"};
...
{
    std::vector<std::string> robot_ids = {"robot1"};  // 일단 1개만
...
void SimpleCentralServer::setupActionClients()
{
    for (const auto& [robot_id, robot_info] : robots_) {
        try {
            // Action client 생성 (단일 로봇이므로 네임스페이스 없음)
...
void SimpleCentralServer::setupNavigationCommandSubscriber()
{
    ...
    RCLCPP_INFO(this->get_logger(), "Send commands like: ros2 topic pub /navigation_command std_msgs/msg/String \"data: 'reception'\"");
}
...
void SimpleCentralServer::navigationCommandCallback(const std_msgs::msg::String::SharedPtr msg)
{
    ...
    // 명령 파싱 (robot_id:waypoint 형식 또는 단순 waypoint)
    std::string robot_id = "robot1";  // 기본값
    std::string waypoint_name = command;
    
    // robot_id:waypoint 형식인지 확인
    ...
    if (waypoint_name == "stop" || waypoint_name == "cancel") {
        // 네비게이션 취소 (실제로는 action cancel 호출 필요)
        std::lock_guard<std::mutex> lock(robots_mutex_);
        if (robots_.find(robot_id) != robots_.end()) {
    ...
    if (waypoints_.find(waypoint_name) != waypoints_.end()) {
        if (sendNavigationGoal(robot_id, waypoint_name)) {
            ...
            RCLCPP_ERROR(this->get_logger(), "Failed to execute navigation command: %s -> %s", 
                        robot_id.c_str(), waypoint_name.c_str());
        }
    } else {
